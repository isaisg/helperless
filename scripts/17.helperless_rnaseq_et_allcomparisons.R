...
### Up ###
sets <- names(lista) %>% grep(pattern = "col_eti",value = T) %>% 
  grep(pattern = "up",value = T)

Res <- NULL
for(set in sets){
  mgenes <- lista[[which(names(lista) == set)]]
...
Res$Set <- Res$Set %>% 
  gsub(pattern = "_up",replacement = "") %>% 
  factor(levels = c("col_eti_4h_s4","col_eti_4h_t2","col_eti_4h_m1","col_eti_8h_s4","col_eti_8h_t2","col_eti_8h_m1"))
...
p_up <- Res %>% 
 ...
 ggplot(data = .,aes(Time,zscore)) +
  ...
  scale_fill_paletteer_d(package = "RColorBrewer",palette = "Accent") +
...
Res$Set <- Res$Set %>% 
  gsub(pattern = "_down",replacement = "") %>% 
  factor(levels = c("col_eti_4h_s4","col_eti_4h_t2","col_eti_4h_m1","col_eti_8h_s4","col_eti_8h_t2","col_eti_8h_m1"))
...
oh.save.pdf(p = composition,outname = "eti_directionality.pdf",outdir = "../figures/",width = 20,height = 8)
...
# 
# #Plot genes
# mgenes <- lista$m_t2_helperdep_8h_up
# melted_sub <- melted[melted$Gene %in% mgenes,] %>% 
#   subset(Genotype == "Col") %>%
#   droplevels
# chibi.boxplot(Map = melted_sub,x_val = "Time",
#               y_val = "zscore",facet_formula = "Treatment",style = "open",alpha_point = 0.1) 
# 
# 
# 
# mgenes <- lista$col_eti_4h_m1_up
# melted_sub <- melted[melted$Gene %in% mgenes,] %>% 
#   subset(Genotype == "Col") %>%
#   droplevels
# chibi.boxplot(Map = melted_sub,x_val = "Time",
